...
public record TokenRefreshResponse(
        @Schema(
                        description = "새로 발급한 Access Token. 인증 API의 Authorization 헤더에 Bearer 접두사와 함께 넣습니다.",
                        format = "JWT",
                        example = "eyJhbGciOiJIUzI1NiJ9...")
                String accessToken,
        @Schema(
                        description = "새로 발급한 Refresh Token. 요청에 사용한 기존 Refresh Token은 폐기되므로 반드시 이 값으로 교체합니다.",
                        format = "JWT",
                        example = "eyJhbGciOiJIUzI1NiJ9...")
                String refreshToken,
