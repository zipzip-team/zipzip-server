...
public record PhotoUploadCompleteRequest(
        @Schema(
                        description = "PHOTO-02 업로드를 완료해 등록할 파일 목록. objectKey 중복 없이 1~20개",
    ...
    public record CompleteFileSpec(
            @Schema(
                            description = "PHOTO-02에서 발급받고 해당 PUT URL로 업로드를 완료한 객체 키",
                            requiredMode = Schema.RequiredMode.REQUIRED)
                    String objectKey,
            @Schema(
                            description = "EXIF에서 추출한 촬영 기기명. 없으면 생략 또는 null, 100자 이하",
                            example = "iPhone 15",
                            maxLength = 100,
                            nullable = true)
                    String deviceModel,
            @Schema(
                            description = "EXIF 촬영일시(UTC ISO-8601). 없으면 생략 또는 null",
