...
- **댓글만 삭제:** `photo_comment` 원본을 즉시 물리 삭제한다([4.6](#46-사진--앨범-nm--삭제-정책)). 다음 타임라인 조회부터 해당 댓글은 반환하지 않는다.
...
- **댓글:** `photo_comment`가 원본이다. 본인 댓글만 수정·삭제할 수 있다(`NOT_PHOTO_COMMENT_AUTHOR`). 삭제는 즉시 물리 삭제하며([4.6](#46-사진--앨범-nm--삭제-정책)), 별도 채팅 이벤트 행을 만들지 않는다. 채팅 타임라인은 조회 시 댓글을 일반 메시지와 병합한다(4.9).
...
| 단계 | 시점 | 내용 |
| --- | --- | --- |
| **MVP** | 앱잼(2주) | 본 문서 전체. 2인스턴스(Docker 단일 API 컨테이너 + PostgreSQL), presigned 업로드(정적 키), 서버 썸네일(JPEG) + DB상태/바운드풀/스윕, N:M 사진-앨범 + soft delete(30일)/댓글·채팅 즉시 삭제 혼합 정책, `HOST`/`MEMBER` 역할, 채팅 통합 타임라인(폴링·조회 병합). WebSocket·Redis Queue 없음. CI/CD = GitHub Actions → Docker Hub(arm64) → SSH 배포. |
