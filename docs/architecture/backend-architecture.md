...
### 4.10 Object Storage 호출 — 전역 동시성 상한

**맥락.** PHOTO-03(완료 등록)은 배치 파일마다 Object Storage에 업로드 완료 여부를 확인한다(`exists()`, HEAD 요청). 파일당 순차 호출이면 배치가 클수록 트랜잭션(DB 커넥션 점유 시간)이 선형으로 늘어나므로, 파일 수(최대 20개)만큼 가상 스레드로 동시에 확인한다. 다만 이 병렬화는 "한 요청 안에서"만 유효하다 — 동시 요청이 여러 건이면 각 요청이 독립된 executor를 열기 때문에, 시스템 전체에서 순간적으로 나가는 HEAD 호출 수는 제한이 없다. 이 호출은 결국 앱 전체가 공유하는 단일 `s3Client` 빈의 HTTP 커넥션 풀을 거치므로, 동시 요청이 몰리면 이 풀이 고갈되어 파일도 예약도 멀쩡한 정상 요청까지 커넥션 획득 실패로 롤백될 수 있다.

**결정.**

- Object Storage HTTP 클라이언트(`Apache5HttpClient`)의 커넥션 풀 크기를 암묵적 SDK 기본값에 맡기지 않고 명시적으로 설정한다(`storage.http-max-connections`, 기본 50). 커넥션 획득 타임아웃도 5초로 짧게 잡아, 풀이 실제로 고갈됐을 때 무한정 매달리지 않고 빠르게 실패한다.
- `PhotoUploadService`(싱글턴 빈)에 요청 간에도 공유되는 전역 `Semaphore(20)`을 둬, 시스템 전체에서 동시에 실행 중인 `exists()` 호출이 이 상한을 넘지 않게 한다. 요청 내부의 파일별 병렬화(가상 스레드)는 그대로 유지하되, 실제 Object Storage 호출 직전에 이 세마포어를 거치게 한다.
- 세마포어 상한(20)은 커넥션 풀(50)보다 넉넉히 낮게 잡아, 같은 `s3Client`를 쓰는 썸네일 워커([4.3](#43-비동기-후처리--db-상태--바운드-풀--스윕-redis-queue-없음), 최대 6 동시)와 삭제 경로(사진·공유 그룹 정리 스윕)가 쓸 여유를 남긴다.

**근거.** 요청 하나의 동시성만 제한하는 것으로는 부족하다 — 공유 자원(HTTP 커넥션 풀)의 상한은 요청 경계와 무관하게 시스템 전체 기준으로 걸어야 한다. 이 문제는 처음엔 인위적 지연(Mockito `Thread.sleep`)으로 재현한 벤치마크에서 드러나지 않았는데, sleep은 유한한 자원을 전혀 점유하지 않기 때문이다 — 실제 커넥션 풀 경합은 코드 리뷰 과정에서 별도로 지적되어 뒤늦게 보강했다. 풀 크기(50)·세마포어 상한(20) 모두 실측이 아니라 배포 규모([6.7](#67-용량--사이징-arm-a1--6gb-기준)) 기준 추정치이므로, 운영 지표를 보며 재조정할 여지가 있다.

세부 벤치마크 수치와 결정 경과는 `docs/apidoc/decisions/api-design-decision-log.md`의 API-14 "후속 조치 현황"을 참고한다.
...
- **Object Storage HTTP 커넥션 풀:** 기본 50(`storage.http-max-connections`), 커넥션 획득 타임아웃 5초. PHOTO-03 완료 등록의 `exists()` 호출은 이 풀을 요청 간에도 나눠 쓰므로, 전역 세마포어(20)로 별도 상한을 둔다([4.10](#410-object-storage-호출--전역-동시성-상한)).
