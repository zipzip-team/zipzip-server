...
- 휴지통 / 최근 삭제 목록: 사진 원본·공유집(앨범)·공유 그룹은 soft delete 후 30일 뒤 물리 정리하되 사용자 복구 UI는 제공하지 않는다. 사진 댓글·그룹 채팅 메시지는 이 유예도 없이 즉시 영구 삭제한다. (자세한 근거는 [4.6](#46-사진-앨범-nm--삭제-정책) 참고)
...
**주의.** 원본 삭제는 복구 불가이므로 iOS 삭제 확인 다이얼로그("원본 삭제"는 "N장 영구 삭제" 경고) 필수. 공유집(앨범)에서 사진을 뺄 때도 그것이 마지막 소속이면 사실상 원본 삭제와 같은 결과이므로, iOS는 제거 전 다른 공유집(앨범) 소속 여부를 확인해 안내하는 것을 권장한다. 사진 삭제는 채팅 피드의 댓글 카드에도 영향(4.9 참조).
...
### 4.9 채팅 = 통합 활동 피드 (댓글 참조 모델)

**맥락.** 채팅창은 단순 메신저가 아니라 **그룹 활동 기록/피드**다. 순수 텍스트 메시지와 "사진에 댓글 달림" 이벤트(사진 썸네일 + photo id + 댓글 내용 카드)가 **같은 스트림에 섞여** 흐른다. 기록용 성격이다.

**결정.**

- 채팅 스트림 `shared_group_chat_message`에 **타입 컬럼**을 추가한다: `type ∈ {TEXT, PHOTO_COMMENT}`.
  - `TEXT`: `content`에 텍스트.
  - `PHOTO_COMMENT`: 원본 텍스트를 복제하지 않고 **참조만** 보관(`photo_id`, `photo_comment_id`).
- **댓글 원본은 `photo_comment` 테이블**(진실의 원천)이다. 채팅의 `PHOTO_COMMENT` 항목은 그것을 가리키는 이벤트 마커일 뿐이다. → **참조 모델 (a)**
- **조회:** 하나의 스트림을 id/시간 순으로 반환(폴링 커서에 유리). `PHOTO_COMMENT` 항목은 `photo`(썸네일 URL) + `photo_comment`(내용·작성자)를 조인해 카드로 채운다.
- **폴링:** 기존 CHAT-01의 cursor 기반 폴링을 그대로 사용한다. TEXT·PHOTO_COMMENT 구분 없이 단일 스트림으로 반환한다.

**근거.** 참조 모델(a)은 댓글 수정/삭제가 한 곳(`photo_comment`)에서 끝나 정합성이 좋다. 채팅 스트림이 자체 id 공간을 가지므로 폴링 증분 조회가 단순하다(두 테이블 UNION의 커서 문제 회피).

**삭제 상호작용(기록용 정책).**

- **댓글만 삭제:** `photo_comment` 원본을 즉시 물리 삭제한다([4.6](#46-사진-앨범-nm--삭제-정책)). 채팅의 `PHOTO_COMMENT` 마커 row는 **남기고** iOS가 "삭제된 댓글" 플레이스홀더로 렌더(참조 무효 처리). 기록이 사라지지 않게 한다.
- **사진 삭제:** 사진과 그 댓글이 제거된다. 채팅의 관련 카드는 **"삭제된 사진" 플레이스홀더**로 렌더한다.
- (기록용 성격상 채팅 항목 자체는 보존하고 플레이스홀더로 표시하는 것을 기본으로 한다. UI 문구·표시 방식은 iOS와 확정한다.)

> **구현 상태:** 이 절의 `type`/`photo_id`/`photo_comment_id` 컬럼과 통합 스트림 조회는 아직 스키마·API에 반영되어 있지 않다. 현재 `shared_group_chat_message`는 순수 텍스트만 저장하고, 사진 댓글은 `docs/apidoc/08-photo-reaction.md`의 별도 COMMENT API로만 접근한다. 이 문서의 결정을 실제로 반영하는 작업은 별도로 진행한다.
...
flowchart TD
    ...
    B -->|앨범에서 제거| C[이 앨범의 shared_album_photo 매핑만 물리 삭제]
    C --> D[다른 앨범 소속·원본 유지 · 완료]
    ...
    I --> J[스윕: 실패분 정리]
    F --> K[채팅 피드의 관련 댓글 카드 → 삭제된 사진 플레이스홀더]
```

앨범에서 제거는 `shared_album_photo` 행만 즉시 물리 삭제하는 동기 동작이다.
원본 삭제는 `photo.deleted_at`을 기록하는 soft delete로 즉시 접근을 차단하고, 30일 뒤 Object Storage 객체 삭제가 성공한 경우에만 DB 행을 물리 삭제한다(DB가 진실의 원천, 스토리지는 결과적 일관 — [1](#1-설계-원칙)). 삭제도 멤버에게 푸시. 관련 채팅 카드는 플레이스홀더 처리(4.9).

### 5.7 좋아요 / 댓글

- **좋아요:** `UNIQUE(photo_id, app_user_id)` 멱등. 중복 시 기존 상태를 그대로 반환(REACTION-02는 멱등 API). 좋아요 시 푸시(묶음·쓰로틀 권장).
- **댓글:** `photo_comment`가 원본. 본인 댓글만 수정·삭제할 수 있다(`NOT_PHOTO_COMMENT_AUTHOR`). 삭제는 즉시 물리 삭제([4.6](#46-사진-앨범-nm--삭제-정책)). 댓글 생성 시 (i) `photo_comment` 저장 → (ii) 채팅 스트림에 `PHOTO_COMMENT` 마커 생성 → (iii) **댓글 이벤트 푸시 1회**(채팅 메시지 푸시로 중복 발송하지 않음). (ii)는 [4.9](#49-채팅--통합-활동-피드-댓글-참조-모델)에서 설명한 대로 아직 구현되어 있지 않다.

### 5.8 채팅 피드 (통합 활동 피드 · 폴링)

```mermaid
flowchart TD
    A[채팅창 입력/행동] --> B{종류}
    B -->|텍스트 입력| C[shared_group_chat_message type=TEXT]
    B -->|사진에 댓글| D[photo_comment 생성 · 원본]
    D --> E[shared_group_chat_message type=PHOTO_COMMENT · 참조 저장]
    C --> S[단일 채팅 스트림]
    E --> S
    S --> P["폴링 GET .../chat-messages?cursor=... · 증분"]
    P --> R{항목 type}
    R -->|TEXT| T[말풍선 렌더]
    R -->|PHOTO_COMMENT| U[photo 썸네일 + comment 조인해 카드 렌더]
    U --> V{참조 유효?}
    V -->|무효 · 삭제됨| W[삭제된 사진/댓글 플레이스홀더]
```

TEXT와 PHOTO_COMMENT가 한 스트림으로 섞여 흐른다. 댓글은 복제하지 않고 참조만([4.9](#49-채팅--통합-활동-피드-댓글-참조-모델)). 채팅방 포그라운드 동안 증분 폴링, 백그라운드에선 폴링이 멈추므로 놓친 메시지는 **FCM 푸시**로 통지(채팅 메시지 푸시는 TEXT에만, 댓글은 댓글 이벤트로만).
...
| 단계 | 시점 | 내용 |
| --- | --- | --- |
| **MVP** | 앱잼(2주) | 본 문서 전체. 2인스턴스(Docker 단일 API 컨테이너 + PostgreSQL), presigned 업로드(정적 키), 서버 썸네일(JPEG) + DB상태/바운드풀/스윕, N:M 사진-앨범 + soft delete(30일)/댓글·채팅 즉시 삭제 혼합 정책, `HOST`/`MEMBER` 역할, 채팅=활동 피드(폴링, 참조 모델은 후속 구현), 푸시(업로드·삭제·입장·퇴장·좋아요·댓글·채팅). WebSocket·Redis Queue 없음. CI/CD = GitHub Actions → Docker Hub(arm64) → SSH 배포. |
| **Phase 2** | 이후 | 콘텐츠 해시 dedup(그룹 스코프) 활성화. 필요 시 Redis 도입(**폴링 채팅 최근 메시지 캐시**가 유력 트리거). 스토리지 인증을 **인스턴스 프린시펄 + PAR** 로 전환. 라이브 포토 영상 페어링. 채팅 통합 활동 피드(`type`/참조 컬럼) 구현. 필요 시 OCIR/GHCR 전환. |
| **Phase 3** | 스케일 정당화 시 | 워커/인스턴스 분리 등. (실시간 요구가 새로 생기면 재검토하되 현재 계획엔 없음.) |

## 9. 열린 항목 (확인 필요)

- **채팅 삭제 UI 문구:** 사진/댓글 삭제 시 채팅 카드를 플레이스홀더로 남기는 정책은 확정(기록용). 표시 문구·스타일은 iOS와 맞춘다.
- **라이브 포토:** MVP는 정지본(JPEG)만. `live_video_object_key`(또는 동등한) 컬럼만 선반영해 Phase 2로 영상 페어링을 여는 방식을 권장하되, 아직 스키마에 반영하지는 않았다.
- **채팅 통합 활동 피드:** [4.9](#49-채팅--통합-활동-피드-댓글-참조-모델)의 `type`/참조 컬럼 설계는 확정했지만 스키마·API에는 아직 반영되지 않았다. 별도 작업으로 진행한다.
