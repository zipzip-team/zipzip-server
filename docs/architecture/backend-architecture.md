...
- **큐 초과 시 안전 처리:** 바운드 풀의 대기열이 가득 차면 제출 시점에 `RejectedExecutionException`이 난다. 업로드 완료 등록의 커밋 후 콜백(`afterCommit`)에서 이 예외를 못 잡으면, 사진은 이미 커밋됐는데 응답만 500으로 깨지는 상태 불일치가 생긴다(#86). 제출 실패는 로그만 남기고 삼키며, 사진은 기본값인 `PENDING`에 남아 위 스윕이 재수거한다 — 최초 제출 경로(`PhotoUploadService`)와 스윕 재제출 경로(`PhotoSweepScheduler`) 모두 동일하게 처리한다.

**근거.** 썸네일 잡은 유실돼도 원본으로 재생성 가능하다. 원본이 이미 영속화돼 있다는 사실을 지렛대 삼아, Redis Queue 없이 DB 상태만으로 "재시작 견딤"을 얻는다. 큐 용량 자체를 실측 기반으로 얼마로 잡을지는 별도 과제다(#107).
