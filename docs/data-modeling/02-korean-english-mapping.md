...
| 공유 공간 | 공유 폴더 | `shared_folder` | - | 테이블, 컬럼, API | 초대 코드, 멤버십, 관리, 채팅의 기준이 되는 공유 공간 |
| 관계 | 공유 폴더 멤버십 | `shared_folder_membership` | - | 테이블, 관계, 권한 설명 | 사용자와 공유 폴더 사이의 참여 관계 |
| 사진 묶음 | 앨범 | `shared_album` | - | 테이블, 컬럼, API | 공유 폴더 하위에 존재 |
| 미디어 | 사진 | `photo` | - | 테이블, 컬럼, API | 앨범 하위에 존재 |
| 미디어 | 촬영일시 | `taken_at` | - | 컬럼, API | nullable. 없으면 `created_at`을 표시·정렬 기준으로 사용 |
| 관계 | 공유 폴더·앨범 생성자 | `created_by_app_user_id` | - | 컬럼, API | 공유 폴더 최초 생성자 이력과 앨범 수정·삭제 권한 기준 |
| 관계 | 사진 업로더 | `uploaded_by_app_user_id` | - | 컬럼, API | 사진 수정·삭제 권한 기준 |
| 사진 반응 | 사진 좋아요 | `photo_like` | - | 테이블, 컬럼, API | 사용자가 특정 사진에 좋아요를 누른 상태 |
| 사진 댓글 | 사진 댓글 | `photo_comment` | - | 테이블, 컬럼, API | 단일 사진에 달리는 댓글 |
| 채팅 | 채팅방 | `chat_room` | - | 테이블, 컬럼, API | 공유 폴더 방장과 멤버 간의 채팅방 |
| 채팅 | 채팅 메시지 | `chat_message` | - | 테이블, 컬럼, API | 채팅방 안에서 작성된 메시지 |
...
| 기기 | 기기 | `device` | - | 테이블, 컬럼, API | 사용자 단위 기기 태그. 공유 폴더 멤버별 표시에 사용 |
| 기기 | 기기 유형 | `device_type` | `CAMERA`, `APP_DEVICE` | 컬럼, 코드값 | 촬영 기기 태그와 앱 설치 기기 태그 구분 |
...
- `shared_album.created_by_app_user_id`는 앨범 수정·삭제 권한의 기준이다.
- `photo.uploaded_by_app_user_id`는 사진 수정·삭제 권한의 기준이다.
- `photo_like`는 사용자가 특정 사진에 좋아요를 누른 상태이다.
- `photo_comment`는 단일 사진에 달리는 댓글이다.
- `chat_room`은 공유 폴더 방장과 멤버가 대화하는 채팅방이다.
- `chat_message`는 채팅방에 작성된 메시지이다.
- `device_type`은 기기 태그가 촬영 기기인지 앱 설치 기기인지 구분한다.
