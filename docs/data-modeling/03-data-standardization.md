...
- 공유 그룹 삭제는 하위 공유집(앨범)·사진에 같은 `deleted_at`을 기록하고, `shared_album_photo` 매핑은 30일 물리 정리 전까지 유지한다. 30일 후 배치는 매핑으로 찾은 사진의 Object Storage 객체를 먼저 삭제하고, 성공한 사진의 관련 행·공유 그룹·초대 코드 예약을 순서대로 물리 삭제한다.
