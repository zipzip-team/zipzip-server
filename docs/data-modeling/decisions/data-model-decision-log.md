...
사진 댓글과 그룹 채팅은 작성·수정·삭제 권한 및 사진 컨텍스트가 달라 저장 모델을 분리해야 한다. 반면 사용자는 공유 그룹 대화 화면에서 두 종류의 대화를 하나의 시간 흐름으로 볼 필요가 있으므로, 조회 모델만 통합한다.
...
6. 사진 업로드 완료 등록은 `objectKey`에 대응하는 `photo_upload_reservation`이 요청 사용자·요청 경로 공유집(앨범)과 일치하고 만료되지 않았는지 확인한다.
