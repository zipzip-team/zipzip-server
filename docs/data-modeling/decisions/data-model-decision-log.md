...
| DM-07 | 그룹 채팅 | 공유 그룹 하나를 채팅방으로 사용한다. 일반 메시지는 `shared_group_chat_message`, 사진 댓글은 `photo_comment`에 별도 저장하고 채팅 조회 시 하나의 타임라인으로 병합한다. |
...
- 공유 그룹 하나가 하나의 채팅방이며, 별도 `chat_room` 테이블이나 채팅방 식별자는 두지 않는다.
- 사진 댓글은 `photo_comment`에 저장한다.
- 사진 컨텍스트 없는 일반 채팅은 `shared_group_chat_message`에 저장한다.
- 채팅 조회는 일반 채팅 메시지와 대상 공유 그룹의 활성 사진 댓글을 `created_at` 기준 단일 타임라인으로 병합한다. 같은 시각은 항목 타입과 항목 ID로 순서를 고정한다.
- 1차 구현은 통합 타임라인의 cursor 기반 폴링 조회를 기준으로 한다.
- MVP는 메시지 작성·조회만 제공하며, 메시지와 사진 댓글의 수정·삭제 API는 후속 범위로 둔다.

근거:

사진 댓글과 그룹 채팅은 사진 컨텍스트가 달라 저장 모델을 분리해야 한다. 사용자는 공유 그룹 대화 화면에서 두 종류의 대화를 하나의 시간 흐름으로 볼 필요가 있으므로, 조회 모델만 통합한다.
...
- 공유집(앨범) 삭제(또는 사진의 명시적 제거)는 관련 `shared_album_photo` 매핑을 즉시 물리 삭제하고, 매핑이 0개가 된 사진도 함께 soft delete한다(DM-06 참고). 공유 그룹 삭제는 하위 공유집(앨범)·사진을 함께 soft delete하되, 30일 뒤 사진 대상을 식별할 수 있도록 `shared_album_photo` 매핑을 유지한다.
...
3. 공유집(앨범) 삭제는 상위 공유 그룹 활성 멤버십만 확인한다(생성자·`HOST` 여부는 확인하지 않는다).
...
6. MVP에는 댓글과 그룹 채팅 메시지의 수정·삭제 API를 제공하지 않는다.
...
1. 사진을 여러 공유집(앨범)에 추가·제거하는 UI 정책을 1차 범위에서 어디까지 지원할지 확인(API는 `PHOTO-06`/`PHOTO-07`로 확정됨)
2. presigned URL TTL, `photo_upload_reservation` 만료 시각, 썸네일 재시도 스윕 주기의 운영 조정 필요 여부 확인

완료:

- 공유 그룹 삭제와 Object Storage 객체 정리 배치의 PostgreSQL 통합 테스트
- presigned URL과 업로드 예약의 기본 TTL 15분, 썸네일 재시도 스윕 기본 주기 5분 적용
