...
- 사진은 반드시 하나의 공유집(앨범)에 속한다.
- 기존 `shared_house` 물리 테이블은 만들지 않고, 공유집(앨범)을 `shared_album`으로 저장한다.
- 기존 `shared_album_photo` 관계 테이블은 만들지 않고, 사진 소속은 `photo.shared_album_id`로 표현한다.
...
- 공유집(앨범) 삭제 시 하위 사진, 사진 반응, 사진 댓글은 조회에서 제외한다.

### 4.4 사진

- 사진은 공유집(앨범)에 업로드된 이미지 원본이다.
- `photo.shared_album_id`가 사진의 직접 소속을 표현한다.
- 사진 업로드 시 대상 공유집(앨범)은 필수다.
- 사진 원본은 원칙적으로 업로더만 수정·삭제할 수 있고, 업로더가 탈퇴한 경우 삭제는 공유 그룹 방장이 할 수 있다.
- 공유 사진을 로컬 저장공간으로 복사하는 플로우는 다운로드이며 서버 엔티티를 새로 만들지 않는다.
- 사진을 다른 공유집(앨범)으로 이동하는 기능이 필요하면 `photo.shared_album_id` 변경 정책을 별도로 정의한다.
...
- 사진 댓글은 단일 사진에 종속되며 작성자만 수정·삭제할 수 있다.

### 4.6 그룹 채팅

- 그룹 채팅은 사진 댓글 모아보기가 아니라 별도 일반 메시지 기능이다.
- 그룹 채팅 메시지는 `shared_group_chat_message`로 저장한다.
...
| `shared_album_photo` | 제외 | 사진은 반드시 하나의 공유집(앨범)에 직접 속한다. |
| `photo_book` | 제외 | 로컬 중심 기능이다. |
| `chat_room` | 제외 | 그룹 채팅은 `shared_group_chat_message`로 충분하다. |
...
4. 사진은 반드시 하나의 공유집(앨범)에 속하며 `photo.shared_album_id`로 표현한다.
...
9. 그룹 채팅 메시지는 `shared_group_chat_message`에 저장하고 1차 구현은 폴링으로 조회한다.
10. 사진 댓글은 `photo_comment`에 저장하며 사진 상세 기능에 속한다.
11. 공유집(앨범) 사진 수는 활성 사진 기준으로 실시간 count한다.
...
- `03-data-standardization.md`: `photo.shared_album_id` 기준의 FK와 삭제 정책을 정의한다.
