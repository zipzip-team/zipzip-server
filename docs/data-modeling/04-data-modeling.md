...
- MVP에서는 사진 댓글과 그룹 채팅 메시지의 작성·조회만 제공하며, 수정·삭제는 후속 범위다.
- 멤버 강제 퇴장은 제공하지 않는다.
...
MVP에서는 메시지 작성과 조회만 제공하며, 수정·삭제는 후속 범위다. 공유 그룹 물리 삭제 시 FK cascade로 정리한다.
...
공유집(앨범) 삭제는 상위 공유 그룹의 활성 멤버라면 생성자·방장 여부와 무관하게 할 수 있다.
...
- 그룹 채팅 메시지와 사진 댓글은 MVP에서 작성·조회만 제공하며, 수정·삭제는 후속 범위다.
- 멤버 강제 퇴장은 제공하지 않는다.
...
- 사진 댓글과 그룹 채팅 메시지의 사용자 삭제 API는 MVP에 포함하지 않는다. 두 행은 사진·공유 그룹의 물리 정리 시 함께 삭제한다.
...
- 개별 삭제한 공유집(앨범)·사진과 삭제된 공유 그룹 데이터는 30일 뒤 물리 삭제한다. 그룹 정리 배치는 후보 스냅샷 수집과 최종 그룹 삭제에서만 PostgreSQL 행 잠금을 짧게 유지한다. 그 사이에는 그룹 트랜잭션 밖에서 `shared_album_photo` 매핑으로 찾은 사진마다 원본·썸네일 Object Storage 객체를 먼저 삭제하고, 성공한 사진의 매핑·댓글·좋아요·사진 행을 사진별 트랜잭션으로 물리 삭제한다.
- 사진 정리 중에는 그룹 행 잠금이 풀리므로 다중 인스턴스가 같은 그룹을 중복 처리할 수 있다. 이미 정리된 사진과 그룹은 건너뛰는 멱등 동작으로 안전하게 종료하고, 최종 삭제 직전에 남은 사진 매핑을 다시 확인한다.
- 그룹 사진 중 하나라도 Object Storage 삭제에 실패하면 해당 사진 트랜잭션만 롤백한다. 앞서 정리를 완료한 사진은 유지하고 공유 그룹과 초대 코드 예약은 남겨, 다음 배치에서 남은 사진부터 재시도한다.
