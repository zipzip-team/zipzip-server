...
사용자와 방장으로 만든 공유 폴더를 soft delete하고, `MEMBER`로 참여 중인 멤버십은 물리 삭제하며, 모든 활성 Refresh Token을 폐기한다. soft delete된 공유 폴더의 `HOST` 멤버십은 공유 폴더 물리 삭제 시 FK cascade로 정리한다. 같은 Apple 계정으로 재가입하면 사용자 정보만 복구하며 과거 멤버십은 자동 복구하지 않는다.
