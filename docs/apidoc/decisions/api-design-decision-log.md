...
| API-14 | 사진 업로드 파일 수·용량 제한 | 요청당 최대 20개, 파일당 최대 20 MiB로 확정(기존 초안값 유지). 21장 이상은 iOS가 20개 단위 배치로 나눠 반복 호출 |
...
- 요청당 업로드 파일 수는 20개, 파일 한 장 최대 크기는 20 MiB로 확정한다(기존 초안값을 그대로 유지).
...
- 배치 크기를 20으로 유지하면 PHOTO-03 한 트랜잭션이 처리하는 사진 생성·썸네일 작업 제출 건수가 선택한 총 사진 수와 무관하게 항상 20건 이하로 고정되어, 트랜잭션 시간과 비동기 썸네일 큐(`thumbnailExecutor`, 스레드 4~6개·큐 100) 부담이 예측 가능하게 유지된다.
- 일반적인 HEIC/JPEG 사진은 대부분 20 MiB 이내라 파일당 용량 상한을 올릴 실익이 크지 않다. 오히려 상한을 올리면 Object Storage 비용과 presigned URL TTL(15분) 내 업로드 실패 위험만 커진다.
