...
| 채팅 | 그룹 채팅 타임라인 조회, 일반 메시지 작성, 수정, 삭제 |
...
사진 댓글과 그룹 채팅은 작성·수정·삭제 권한 및 사진 컨텍스트가 달라 저장 모델을 분리한다. 다만 공유 그룹 대화 화면에서는 두 종류의 대화를 시간순으로 함께 보여줘야 하므로, 저장 모델은 분리하고 조회 모델만 통합한다.
