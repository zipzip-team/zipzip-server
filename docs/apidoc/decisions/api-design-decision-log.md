...
| API-10 | 그룹 채팅 | 공유 그룹 하나를 채팅방으로 사용하고, 일반 메시지와 사진 댓글은 별도 저장·통합 조회 |
...
| 앨범 | 공유집(앨범) CRUD |
| 사진 | 공유집(앨범) 사진 목록, 업로드 URL 발급·완료 등록, 메타데이터 수정, 앨범 추가·제거 |
| 사진 반응·댓글 | 사진 상세, 좋아요, 사진 댓글 |
| 채팅 | 그룹 채팅 타임라인 조회, 일반 메시지 작성 |
...
- 공유 그룹 하나가 하나의 채팅방이다. 별도 채팅방 조회 API와 `chat_room` 테이블을 두지 않고, `sharedGroupId`를 채팅방 식별자로 사용한다.
- 사진 댓글은 `photo_comment`에 저장한다.
- 사진 컨텍스트 없는 그룹 채팅 메시지는 `shared_group_chat_message`에 저장한다.
- CHAT-01은 일반 메시지와 대상 공유 그룹의 활성 사진 댓글을 단일 타임라인으로 병합해 반환한다. 각 항목은 `CHAT_MESSAGE` 또는 `PHOTO_COMMENT` 타입을 가지며, 사진 댓글은 연결된 사진 식별자인 `photoId`를 함께 반환한다.
- 타임라인은 `createdAt` 내림차순, 항목 타입, 항목 ID 순으로 고정하고 cursor는 세 값을 모두 포함하는 불투명 값으로 정의한다.
- 1차 구현은 통합 타임라인의 cursor 기반 폴링 조회를 기준으로 한다.
- MVP는 메시지 작성·조회만 제공하며, 메시지와 사진 댓글의 수정·삭제 API는 후속 범위로 둔다.

### 판단 이유

사진 댓글과 그룹 채팅은 사진 컨텍스트가 달라 저장 모델을 분리한다. MVP에서는 두 종류의 대화를 시간순으로 함께 보여줘야 하므로, 저장 모델은 분리하고 조회 모델만 통합한다.
...
## 16. API-14. 사진 업로드 파일 수·용량 제한

### 결정

- 요청당 업로드 파일 수는 20개, 파일 한 장 최대 크기는 20 MiB로 유지한다(기존 초안값을 정량적으로 재검증 후 확정).
- 21장 이상 선택은 서버가 처리하지 않고, iOS가 20개 단위 배치로 나눠 PHOTO-02→PUT→PHOTO-03을 반복 호출해 처리한다.
- 이번 재검증으로 발견한 실제 위험 요인들은 20개/20MiB라는 숫자를 조정해서는 해소되지 않는다고 판단해, 값 변경 대신 별도 이슈로 분리한다.

### 실측 근거

이슈 #70을 작업하기 위해 배포 인스턴스(Oracle Cloud Always Free ARM Ampere A1, 6GB, prod·dev 컨테이너가 cgroup 메모리 제한 없이 한 인스턴스를 공유)를 기준으로 두 갈래를 직접 측정했다(`ThumbnailMemoryBenchmarkTest`, `PhotoUploadBatchLoadBenchmarkTest`, 둘 다 `-PincludeBenchmark`로 실행하는 조사용 벤치마크로 기본 `test` 태스크에서는 제외).

**파일 크기 → 썸네일 생성 메모리**

| 조건 | 파일 크기 | 피크 메모리 증가량 |
|---|---|---|
| 12MP, quality 0.15~1.00 (5개 표본) | 0.31~12.42MiB | 71~74MiB (거의 고정) |
| 12MP | 1.34MiB | 73.6MiB |
| 48.8MP | 0.76MiB (더 작음) | 252.4MiB (3.4배 더 큼) |
| 12MP × 동시 6개(스레드풀 최대치) | 12.42MiB | 384.5MiB (합산) |

파일 크기를 40배 이상 바꿔도 메모리 증가량은 거의 고정인 반면, 해상도를 4배로 올리자 파일 크기가 더 작았음에도 메모리는 3.4배 커졌다. 즉 메모리 비용은 압축 파일 크기가 아니라 해상도(픽셀 수)가 결정하며, `sizeBytes` 상한(20MiB)은 이 비용을 통제하지 못한다. 해상도(`width`/`height`) 자체는 시스템 어디에도 검증·상한이 없다(이슈 #87로 분리).

**배치 크기 → 완료 등록(PHOTO-03) 트랜잭션 시간과 썸네일 큐**

| 배치 크기 | 완료 등록 소요 시간(Object Storage 지연 80ms 시뮬레이션) | 파일당 평균 |
|---|---|---|
| 5개 | 689ms | 137.8ms |
| 10개 | 970ms | 97.0ms |
| 20개 | 1959ms | 98.0ms |

파일당 처리 시간이 배치 크기와 무관하게 거의 고정(~97~98ms)이라는 건, 병목이 배치 크기가 아니라 `completeUpload`의 `exists()` 순차 호출 구조라는 뜻이다(이슈 #88로 분리). 동시 사용자 3명이 각자 20개 배치를 완료 등록했을 때 `thumbnailExecutor`(core=4, max=6, 큐 100 — 전부 미검증 하드코딩 기본값) 대기 큐가 즉시 56/100까지 찼다. 이 서비스는 같은 모임에서 여러 명이 비슷한 시점에 몰아서 업로드하는 패턴이 자연스러워, 사용자 5~6명만 겹쳐도 큐 초과 시나리오가 드문 예외가 아닐 수 있다(이슈 #86으로 분리 — 큐 초과 시 이미 성공한 업로드의 응답이 500으로 깨질 위험 포함).

### 판단 이유

- 사진 선택 UX에서 "무제한 선택"처럼 보이게 하는 것과 서버 상한을 올리는 것은 별개 문제다. iOS가 배치 단위(20개)로 나눠 순차·병렬로 업로드하고 하나의 진행률만 노출하면, 서버 상한을 올리지 않고도 동일한 사용자 경험을 만들 수 있다.
- **파일 크기 상한을 조정해도 실제 위험(해상도 기반 메모리 비용)은 줄지도 늘지도 않는다** — 이미 20MiB 이하인 고해상도 이미지로도 같은 위험이 존재하므로, 이 숫자는 애초에 틀린 손잡이다. 값은 유지하고 해상도 자체를 통제하는 별도 안전장치(이슈 #87)로 대응한다.
- **배치 크기를 올리면 이미 확인된 위험(트랜잭션 시간, 큐 점유)이 더 나빠진다** — 20개 배치가 이미 트랜잭션 약 2초, 동시 3명만으로 큐 절반 이상을 소모하므로, 올릴 근거가 없다. 줄이는 것도 근본 병목(`exists()` 순차 호출)을 해결하지 못해 효과가 제한적이다. 값은 유지하고 병목 자체(이슈 #88)와 큐 초과 시 안전한 실패(이슈 #86)를 별도로 고친다.
