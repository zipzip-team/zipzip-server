...
| 항목 | 값 |
|---|---|
| 한 요청의 최대 사진 수 | 20개. 21장 이상 선택은 iOS가 20개 단위 배치로 나눠 PHOTO-02→PUT→PHOTO-03을 반복 호출한다([배치 분할 가이드](shared-album-photo-ios-integration-guide.md#21장-이상-선택-시-배치-분할) 참고) |
...
파일 수·크기 제한은 UX·운영 비용 검토 후 확정했다(근거: [의사결정 로그 API-14](decisions/api-design-decision-log.md#16-api-14-사진-업로드-파일-수용량-제한)). 제한값이 바뀌어도 DB 스키마 변경은 필요하지 않다.
